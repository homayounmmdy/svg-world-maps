...
if (args.length !== 1) {
    console.error(`
Usage:
  node scripts/fill-the-states.mjs <mapName> [--dry]

Examples:
  node scripts/fill-the-states.mjs turkey
  node scripts/fill-the-states.mjs TURKEY
  node scripts/fill-the-states.mjs src/maps/optional/turkey.ts

Expected flat structure:
  src/maps/optional/TURKEY.ts
  src/maps/optional/map.svg
`);
    process.exit(1);
}
...
function resolveInput(input) {
    ...
    // If user passed the real .ts file directly.
    if (isFile(resolved)) {
        ...
        const svgFile = findDirectFile(dir, "map.svg");

        if (!svgFile) {
            throw new Error(`Found ${resolved}, but could not find map.svg in ${dir}.`);
        }
    ...
    // Remove .ts if user passed "turkey.ts".
    const requestedPath = String(input).replace(/\.ts$/i, "");

    // If user passed a path like src/maps/optional/turkey
    if (requestedPath.includes("/") || requestedPath.includes("\\")) {
        const resolvedRequestedPath = path.resolve(cwd, requestedPath);
        const root = path.dirname(resolvedRequestedPath);
        const name = path.basename(resolvedRequestedPath);

        const tsFile = findTsFileDirect(root, name);

        if (tsFile) {
            const svgFile = findDirectFile(root, "map.svg");

            if (!svgFile) {
                throw new Error(`Found ${tsFile}, but could not find map.svg in ${root}.`);
            }
    ...
    // If user passed only the map name: turkey, TURKEY, etc.
    const mapName = path.basename(requestedPath).replace(/\.ts$/i, "");
    ...
    if (!roots.length) {
        throw new Error(
            "No map folder found. Looked for src/maps/optional, root/src/maps/optional, maps/optional, maps, and current directory."
        );
    }

    let foundTsWithoutSvg = null;

    for (const root of roots) {
        const tsFile = findTsFileDirect(root, mapName);
    ...
    if (foundTsWithoutSvg) {
        throw new Error(
            `Found ${foundTsWithoutSvg.tsFile}, but could not find map.svg in ${foundTsWithoutSvg.root}.`
        );
    }

    throw new Error(
        `Could not find ${mapName}.ts in src/maps/optional or other searched folders.`
    );
}

function findTsFileDirect(dir, mapName) {
    if (!isDirectory(dir)) {
        return null;
    }

    const target = String(mapName)
        .replace(/\.ts$/i, "")
        .toLowerCase();
    ...
    // Exact file name match: turkey.ts / TURKEY.ts / Turkey.ts
    const exact = entries.find(
        (entry) => entry.name.toLowerCase() === `${target}.ts`
    );
    ...
    // Fallback: any .ts file whose base name matches.
    const byBaseName = entries.find((entry) => {
        const fileName = entry.name.toLowerCase();

        return (
            fileName.endsWith(".ts") &&
            !fileName.endsWith(".d.ts") &&
            baseNameWithoutExt(entry.name).toLowerCase() === target
        );
    });

    if (byBaseName) {
        return path.join(dir, byBaseName.name);
    }

    return null;
}

function findDirectFile(dir, fileName) {
    if (!isDirectory(dir)) {
        return null;
    }
    ...
    const entry = listFiles(dir).find(
